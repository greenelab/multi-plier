...
# for the NARES LVs, what recount2 LVs are they most correlated with (max)?
nares.max.cor.lv <- apply(cor.z.mat[1:(ncol(recount.z) - 1), 
                                 ncol(recount.z):ncol(cor.z.mat)], 
                          2, which.max)

# correlation values themselves
nares.max.cor <- apply(cor.z.mat[1:(ncol(recount.z) - 1), 
                                 ncol(recount.z):ncol(cor.z.mat)], 
                       2, max)

# bind LV names together
mapping.df <- as.data.frame(cbind(names(nares.max.cor.lv), 
                                  colnames(cor.z.mat)[nares.max.cor.lv],
                                  nares.max.cor))
colnames(mapping.df) <- c("NARES_LV", "recount_LV", "pearson_Z")
```
...
rownames(nares.b) <- paste("NARES", rownames(nares.b))
rownames(recount.nares.b) <- paste("recount", rownames(recount.nares.b))

# correlation
cor.b.mat <- cor(t(nares.b), t(recount.nares.b))

# add the LV (B) values to mapping data.frame
mapping.df$pearson_B <- rep(NA, nrow(mapping.df))
for (row.iter in 1:nrow(mapping.df)) {
  recount.index <- as.integer(gsub("recountLV", "", 
                                   mapping.df$recount_LV[row.iter]))
  mapping.df[row.iter, "pearson_B"] <- cor.b.mat[row.iter, recount.index]
}
...
mapping.df <- mapping.df %>%
                dplyr::mutate(pearson_Z = as.numeric(as.character(pearson_Z)))
mapping.df
```
...
gplots::heatmap.2(cor.map.mat,
                  ...
                  RowSideColors = row.color.bar)
legend("topright", 
       legend = c("significant"),
       col = c("#0000FF"), cex = 0.5,
       lty = 1, lwd = 6)
```

```{r}
# save plot to file
pdf(file.path(plot.dir, "NARES_Z_B_correlation_heatmap.pdf"))
gplots::heatmap.2(cor.map.mat,
                  Rowv = NA, Colv = NA, trace = "none", dendrogram = "none",
                  col = colorRampPalette(colors = c("#FFFFFF", "#FF0000")),
                  cellnote = round(x = cor.map.mat, 2), notecol = "#000000",
                  notecex = 0.6, margins = c(5, 9),
                  cexCol = 0.75, cexRow = 0.7,
                  RowSideColors = row.color.bar)
legend("topright", 
       legend = c("significant"),
...
```{r}
# tidy things up for plotting
b.cor.df <- reshape2::melt(cor.b.mat)
head(b.cor.df)
```

```{r}
colnames(b.cor.df) <- c("NARES_LV", "recount_LV", "Correlation")
b.cor.df$LV_type <- rep("all", nrow(b.cor.df))
```
...
colnames(best.match.df)[3] <- "Correlation"
b.cor.df <- dplyr::bind_rows(b.cor.df, best.match.df) 
```

```{r}
# we'll add points for the significant lvs in blue
sig.pts <- best.match.df$Correlation[sig.lvs]
```
...
b.cor.df %>%
  ...
  ggplot2::theme_bw() +
  ggplot2::annotate("point", x = sig.pts, y = rep(0, length(sig.lvs)),
                    shape = 15, size = 3, colour = "#0000FF", alpha = 0.6) +
  ggplot2::labs(title = "Correlation NARES B matrices",
                subtitle = "NARES v. recount2 PLIER models") +
