...
```{r}
# plots directory specifcally for this notebook
...
Now, consider each "significant" LV.
...
ggplot2::ggplot(as.data.frame(u.sparsity.sig),
  ...
  ggplot2::theme_bw() +
  ggplot2::labs(x = "proportion of positive entries in U") +
  ggplot2::ggtitle("Pathway-associated LVs")
```
```{r}
png.file <- file.path(plot.dir, 
                      "recount2_prop_pos_entries_U_significant_lvs.png")
ggplot2::ggsave(filename = png.file, plot = ggplot2::last_plot(), 
                width = 7, height = 5, units = "in")
```
...
# tidy format
recon.eval.df <- 
  rbind(cbind(recon.error, recon.cor, rep("All", length(recon.error))),
        cbind(sig.recon.error, sig.recon.cor, rep("Pathway-associated", 
                                                  length(sig.recon.error))))
colnames(recon.eval.df) <- c("MASE", 
...
ggplot2::ggplot(recon.eval.df,
  ...
  ggplot2::facet_wrap(~ `LVs used in reconstruction`) +
  ggplot2::theme_bw() +
  ggplot2::labs(title = "Input vs. PLIER reconstructed recount2 data") +
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, face = "bold"))
```
