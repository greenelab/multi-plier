...
GetGeneSetMean <- function(gene.set, exprs) {
  ...
  gene.set.exprs <- 
    exprs[which(exprs$Gene %in% gene.set), 2:ncol(exprs)]
  summary.df <- gene.set.exprs %>%
...
sle.b.df <- sle.b.df[, c("Sample", "LV", "Annotated", "Value")]
...
array.att <-
  ...
  dplyr::mutate(`Disease state` =
                  dplyr::recode(array.att$`Disease state`, 
...
sle.b.meta.df <- dplyr::right_join(sle.b.df, array.att, by = "Sample") %>%
  dplyr::filter(LV %in% c("LV6", "LV38", "LV41"))

# write to file
sle.b.file <- file.path("results", "E-GEOD-78193_SLE-WB_PLIER_IFN_B.tsv")
...
rec.b.meta.df <- dplyr::right_join(recount.b.df, array.att, by = "Sample") %>%
  dplyr::filter(LV %in% c("LV35", "LV1070"))

# write to file
rec.b.file <- file.path("results", "E-GEOD-78193_recount2_PLIER_IFN_B.tsv")
