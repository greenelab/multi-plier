...
MajorityRules <- function(list.of.plier.models) {
  
  # what pathways have at least one LV with FDR < 0.05
  IdentifyCapturedPathways <- function(plier.results) {
    summary.df <- plier.results$summary
    captured.pathways <- 
      # TODO: add fdr.cutoff arg
      unique(summary.df$pathway[which(summary.df$FDR < 0.05)])
  }
  ...
  majority.pathways <- names(count.table[which(count.table >= 3)])
...
Specifically for this notebook
...
```{r}
eight.thousand.pathways <- MajorityRules(eight.thousand.list)
```
