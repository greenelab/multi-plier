...
```{r}
# for use with ggplot2::stat_summary
CISummary <- function(x) {
  # Mean, 95% CI
...
MultiPlotWrapper <- function(lv.name) {
  ...
  nares.p <- nares.df %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x = Classification, y = Value)) +
      ...
      ggplot2::stat_summary(geom = "pointrange", fun.data = CISummary) +
  ...
  #### Glomeruli ####
  glom.p <- kidney.df %>% 
    dplyr::filter(LV == "524,IRIS_Neutrophil-Resting") %>%
