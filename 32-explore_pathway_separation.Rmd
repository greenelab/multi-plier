...
```{r}
# detect for pathway separation
context.results <- lapply(context.model.files,  # for each context (5x repeat)
                       function(x) {
                         # read in the list of 5 models
                         models.list <- readRDS(x)
                         lapply(models.list, 
...
# melt and bind the pathway separation results, using the sample size as an
# identifier
context.results.df <- dplyr::bind_rows(lapply(context.results, 
                                           reshape2::melt), 
                                    .id = "biological_context")
colnames(context.results.df)[3:4] <- c("pathway", "seed")
```

Wrangle data for heatmap

```{r}
context.df <- context.results.df %>%
  # for each biological context, pathway pair
  dplyr::group_by(biological_context, pathway) %>%
  # count how many models where there's separation
  dplyr::summarize(model_count = sum(value)) %>%
  # spread the columns
  tidyr::spread(biological_context, model_count) %>%
  # reorder such that sample size is increasing
  dplyr::select(c("pathway", "blood", "cancer", "tissue", "cell_line", 
                  "other_tissues")) %>%
  # the pathway names should be rownames rather than an individual column
  tibble::column_to_rownames("pathway") %>%
  as.data.frame()
